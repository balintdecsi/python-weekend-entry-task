        ...
        for line in f:
            ...
            if counter == 1:
                ...
                all_flights.append(
                    (record[1],
                    record[2], record[3],
                    dt.fromisoformat(record[4]), dt.fromisoformat(record[5]),
                    float(record[6]),
                    float(record[8]), int(record[10])))
...
def plan_route(
    ...
    flight_combination -- dictionary with flights previosly taken into accoutn at the given point of the route
...
if __name__ == '__main__':
    ...
    with open("balintdecsi_output.json", "w") as f:
        dump(sorted(output_flights, key=lambda x: x["total_price"]), f, indent="\t")